...
class Hyperparams:
    def __init__(self, **kwargs):
        ...
        self.special_hidden: coordinates.Coords | None = (
            ...
            if kwargs.get('special_hidden_spot') else None)
         

        # Boolean settings
        self.menhir_ignore: bool = kwargs.get('menhir_ignore', False)
        self.mist_ignore: bool = kwargs.get('mist_ignore', False)
        self.ignore_possible_attack_opportunity: bool = kwargs.get('ignore_possible_attack_opportunity', False)
...
class ThompsonSampling:
    REWARDS = [0, 1, 2, 3, 4, 5, 7, 9, 12, 17, 23, 30, 41, 55, 74, 99, 133, 178, 
               239, 320, 429, 575, 770, 1031, 1381, 1850, 2479, 3320, 4447, 5956, 
               7977, 10685]
    ...
    def select_arm(self) -> tuple[str, Hyperparams]:
        ...
        sampled_values = {arm: np.random.beta(self.successes[arm], self.failures[arm]) 
...
class Rustler(controller.Controller):
    ...
    def go_to_hidden_spot(self, knowledge: characters.ChampionKnowledge):
        dst, cords = min([(utils.norm(spot - knowledge.position), spot) for spot in self.HIDING_SPOTS], key=lambda x: x[0])
    ...
    def initial_find_weapon(self, knowledge):
        ...
        if self.weapon.name == 'knife':
            weapons_list = self.path_finder.letters_position(knowledge.position.x, knowledge.position.y, self.curr_params.blacklisted_letters)
            self.curr_targets_set.add(Goal('weapon', 1, coordinates.Coords(weapons_list[0][0], weapons_list[0][1]), True))
    ...
    def decide(self, knowledge: characters.ChampionKnowledge) -> characters.Action:
        try:
            ...
            if not self.spawned:
                ...
                if not self.curr_params.disable_hidden_spots:
                    if self.curr_params.special_hidden:
                        self.curr_targets_set.add(Goal('hide', 1000, self.curr_params.special_hidden, self.curr_params.explore))
                    else:
            ...
            for target in self.curr_targets_set:
                if target.vanishable and target.journey_target.x == knowledge.position.x and target.journey_target.y == knowledge.position.y \
            ...
            if not self.curr_params.ignore_possible_attack_opportunity and len(possible_targets) > 0:
                #print("targets:", possible_targets)
                targets_distance :list[int, coordinates.Coords] = [(utils.norm(coords - knowledge.position), coords) for coords in possible_targets]
                min_dist_to_champion, closest_chamption_coords = min(targets_distance, key=lambda x: x[0])
            ...
            if len(self.curr_targets_set) > 0:
                ...
                path = self.path_finder.shortest_path(knowledge.position.x, knowledge.position.y, curr_target.journey_target.x, curr_target.journey_target.y)
                facing_action = self.determine_target_facing(knowledge.position, curr_target)
